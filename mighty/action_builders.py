...
def action_from_recommendation(recommendation: Any) -> Action:
    ...
    return Action(
        ...
        reasoning=reasoning,
        evidence=list(getattr(recommendation, "evidence", None) or []),
        why_now=str(getattr(recommendation, "why_now", "") or "").strip(),
        alternative_options=list(getattr(recommendation, "alternative_options", None) or []),
